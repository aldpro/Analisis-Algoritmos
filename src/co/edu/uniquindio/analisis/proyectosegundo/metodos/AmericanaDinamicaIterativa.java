...
public class AmericanaDinamicaIterativa {
	...
	public ArrayList<Integer> multiplicacionAmericana(ArrayList<Integer> arreglo1, ArrayList<Integer> arreglo2) {
		// Obtiene el tama?o de los dos arreglos de entrada
		int n = arreglo1.size();
		int m = arreglo2.size();
		...
		for (int i = n - 1; i >= 0; i--) {
			...
			for (int j = m - 1; j >= 0; j--) {
				// Realiza la multiplicaci?n de los elementos correspondientes
				...
				// Si el resultado de la multiplicaci?n es mayor o igual a 10, lleva el excedente al elemento anterior del ArrayList resultado
				resultado.set(i + j, resultado.get(i + j) + resultado.get(i + j + 1) / 10);
				// Guarda el resultado de la multiplicaci?n en el elemento correspondiente del ArrayList resultado
